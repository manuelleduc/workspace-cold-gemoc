...
class ExplicitOperationsFormatter extends AbstractFormatter2 {
	...
	def dispatch void format(Model model, extension IFormattableDocument document) {
		...
		for (FeatureModel fm : model.featureModels) {
			fm.format;
		}
	}
	...
	def dispatch void format(Composition composition, extension IFormattableDocument document) {
		...
		interior(
			...
			composition.regionFor.keyword(']').append[newLine],
			[indent]
		)
	...
	def dispatch void format(SomeOf someOf, extension IFormattableDocument document) {
		someOf.regionFor.keyword('?').prepend[noSpace].append[oneSpace]
		someOf.regionFor.keyword('(').prepend[oneSpace].append[noSpace]
		someOf.regionFor.keyword(')').prepend[noSpace].append[noSpace]
		interior(
			someOf.regionFor.keyword('[').prepend[oneSpace].append[newLine],
			someOf.regionFor.keyword(']'),
			[indent]
		)
	...
	def dispatch void format(Equiv equiv, extension IFormattableDocument document) {
		equiv.regionFor.keyword('<=>').surround[oneSpace]
	}

	def dispatch void format(Imply imply, extension IFormattableDocument document) {
		imply.regionFor.keyword('=>').surround[oneSpace]
	}

	def dispatch void format(Or or, extension IFormattableDocument document) {
		or.regionFor.keyword('or').surround[oneSpace]
	}

	def dispatch void format(And and, extension IFormattableDocument document) {
		and.regionFor.keyword('and').surround[oneSpace]
	}
